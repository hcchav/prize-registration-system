...
// Removed Wheel import since we don't need it anymore
...
export default function Home() {
  ...
  // Removed confetti effect since we don't need it for the signup page
  ...
  // No longer need handleSpinComplete since we removed the wheel
  useEffect(() => {
    // When reaching step 3, automatically show the claim number
    if (step === 3) {
      // No need to set any prize-related state
      setLoading(false);
    }
  }, [step]);
          ...
          <div className="bg-white w-[375px] h-auto relative">
            <div className="w-80 h-auto mt-5 mx-auto rounded-[5px] border-2 border-solid border-[#abcae9] overflow-hidden">
              ...
              <div className="flex flex-col w-80 items-center justify-center gap-3 px-6 py-8">
                ...
                <div className="w-full bg-[#f5f9fd] p-4 rounded-md border border-[#abcae9] text-center">
                  <p className="text-[#00263a] text-sm mb-1">Your Claim Number</p>
                  <p className="text-[#418FDE] text-2xl font-bold">
                    {formatRegNumber(localStorage.getItem('claimId') || localStorage.getItem('attendeeId') || 'N/A')}
                  </p>
                </div>
                
                <p className="text-[#666] text-xs mt-4 text-center">
                  Please show this claim number at the booth to spin the wheel and win your prize.
                </p>
              </div>
            </div>
          </div>
