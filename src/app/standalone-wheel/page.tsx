...
import StandaloneWheelDesktop from '@/components/standalone-wheel-desktop';
...
export default function WheelPage() {
  ...
  const verifyClaimNumber = async () => {
    ...
    try {
      ...
      // Check if claim number is 9999
      if (claimNumber.trim() === '9999') {
        // Generate a random attendee ID
        const randomAttendeeId = `user-${Math.floor(Math.random() * 10000)}`;
        
        // Store attendee ID in state and localStorage
        setAttendeeId(randomAttendeeId);
        localStorage.setItem('attendeeId', randomAttendeeId);
        ...
        setError('Invalid claim number. Please enter 9999.');
        setVerifying(false);
        return false;
      }
    } catch (err) {
