...
import './standalone-wheel-desktop.css'; // Using our desktop-specific CSS file
...
export default function WheelDesktop({ onSpinStart, onSpinComplete, onError, testMode = false, aboveButtonContent }: WheelProps) {
  ...
  const handleSpin = useCallback(async () => {
    ...
    try {
      ...
      if (onSpinStart) {
        try {
          ...
          // If result is explicitly false, don't proceed
          if (result === false) {
            setLoading(false);
            return;
          }
        ...
        // We've already handled the cases where we shouldn't proceed above
      }
      ...
      try {
        ...
        response = await fetch(`/api/standalone-wheel-assign-prize?t=${timestamp}`, {
          method: 'POST',
          headers: {
    ...
    //   }
      console.log('Selected prize:', assignedPrizeData);
